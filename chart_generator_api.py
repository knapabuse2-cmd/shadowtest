...
# Маппинг таймфреймов для chart-img.com v2
# ВАЖНО: часовые интервалы как "1h", "4h", а не числа!
TF_MAP = {
    "1m": "1",
    "3m": "3",
    "5m": "5", 
...
class ChartGeneratorAPI:
    ...
    def __init__(self, api_key: str = None, width: int = 600, height: int = 400):
    ...
    async def generate(self, signal: SignalData) -> Optional[bytes]:
        ...
        tv_interval = TF_MAP.get(signal.tf.lower(), "1h")
    ...
    def _build_drawings(self, signal: SignalData, is_long: bool) -> list:
        ...
        ВАЖНО: 
        - name: "Horizontal Line" (с пробелом и заглавными!)
        - override вместо options
        - lineColor как rgb()
        """
        drawings = []
...
class ChartGenerator:
    ...
    def __init__(self, api_key: str = None, width: int = 600, height: int = 400, **kwargs):
...
if __name__ == "__main__":
    ...
    async def test():
        gen = ChartGeneratorAPI(width=600, height=400)
